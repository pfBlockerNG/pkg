...
``PFB_SRC``, defaulting to ``../pfBlockerNG`` relative to this repo's root, and raises
a clear error before any test runs if the engine checkout is missing or incomplete.
"""
...
_PKG_REPO_ROOT = Path(__file__).resolve().parent.parent
...
def resolve_src_root() -> Path:
    ...
    root = Path(raw).expanduser() if raw else (_PKG_REPO_ROOT.parent / "pfBlockerNG")
    required = (
